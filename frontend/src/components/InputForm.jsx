...
const InputForm = ({ onInputReady }) => {
    ...
    const getOverlayBlur = (type) => {
        if (activeInput && activeInput !== type) return 'opacity-40 pointer-events-none grayscale';
        return '';
    };
    ...
    return (
        ...
        >
          ...
          ${getOverlayBlur('image')}`}
                ...
                onDrop={handleDrop}
                onClick={(e) => {
                    // Prevent triggering if clicked from input's clear event bubble
                    if (!activeInput && fileInputRef.current) {
